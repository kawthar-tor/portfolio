    ...
    <div className='flex flex-col items-center'>
      ...
      <div className='flex justify-center flex-wrap gap-10'>
      {
        (activeFilter === 'All' ? projects : projects.filter(project => project.tags.find(tag => tag.name === activeFilter))).map((project, index) => (
          <div key={index} className={`flex gap-3 hover:shadow-card hover:scale-[1.02] animate-fade transition duration-700 ease-in-out w-1/4 flex-col items-center overflow-hidden p-7 rounded-tl-3xl rounded-br-3xl bg-navy text-primary cursor-pointer`}>
